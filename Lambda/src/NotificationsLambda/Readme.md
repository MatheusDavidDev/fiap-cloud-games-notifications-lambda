...
WORKDIR /src
COPY ["NotificationsLambda.csproj", "NotificationsLambda/"]
RUN dotnet restore "NotificationsLambda/NotificationsLambda.csproj"

WORKDIR "/src/NotificationsLambda"
COPY . .
RUN dotnet build "NotificationsLambda.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "NotificationsLambda.csproj" \
...
Execute unit tests
```
cd "NotificationsLambda/test/NotificationsLambda.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "NotificationsLambda/src/NotificationsLambda"
